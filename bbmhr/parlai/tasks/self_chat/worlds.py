...
from bbmhr.pipeline.prompting import load_large_model, inference
...
class SelfChatWorld(DialogPartnerWorld):
    def __init__(self, opt, agents, shared=None):
        ...
        self.turn_cnt = 0
        self.episode_cnt = 0
        # process bbmhr setting
        self.history = ""
        if self.opt.get("use_reasoning"):
    ...
    def _get_seed_utt_acts(
        ...
        if self.turn_cnt == 0:
            ...
            initial_utterances = []
            for i in range(len(self._openers)):
                initial_utterances.append(
                    make_agent_action(self._openers[i], agents[i%2])
                )
            return initial_utterances
        else:
    ...
    def parley(self):
        ...
        print(f"seed utterances:{self.seed_utterances}")
        if self.contexts:
            ...
            if self.opt.get("use_reasoning"):
                for i in [0, 1]:
                    ...
                    print(f"World acts: {self.acts}")
                    if i == 1:
                        self.history += "supporter: " + self.acts[1]['text'] + "\n"
                    elif i == 0:
                        self.history += "seeker: " + self.acts[0]['text'] + "\n"
                        gpt_response = ""
                        ...
                        self.acts[0]['text'] = self.acts[0]['text'] + gpt_response
            ...
            if self.opt.get("use_reasoning"):
                self.history += "seeker: " + acts[0]['text'] + "\n"
                gpt_response = ""
                gpt_response = inference(
                    self.opt.get("reasoning_model_name"),
                    self.model,
                ...
                acts[0] = Message(
                    {
                        'text': acts[0]['text'] + gpt_response, 
                        'episode_done': acts[0]['episode_done'], 
                        'id': acts[0]['id']}
                )
                # set('text', acts[0]['text'] + gpt_response)
                # acts[0]['text'] = acts[0]['text'] + gpt_response
                print(f"acts[0] after prompting: {acts[0]}")
            agents[1].observe(validate(acts[0]))
            acts[1] = agents[1].act()
            self.history += "supporter: " + acts[1]['text'] + "\n"
